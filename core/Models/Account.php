	     	...
	     	$queryC->execute();
	      	...
	      	//Insert And Register Member
		       ...
		       if($lastInsertId){
					...
					$message="Hi ".$fname.", "."Welcome to {$this->sitename}. At {$this->sitename}, Access all our services. Our customer support line is available to you 24/7. Stay connected.";
					...
					State: $state</h3>";
					$message2.="<br/><br/><br/> <i>Notification Powered By SpecialsuData</i>";
		...
		public function verifyAndUpdateNinOnMonnify($kycData){
			...
			if($shouldupdateStatus == "success"){
				...
				$sql = "
					...
					UPDATE subscribers SET sWallet=@newwallet,sKycStatus='verified',sNin=:nin,sDob=:dob,sAccountLimit = '10000' WHERE sId=:id;
		...
		public function verifyAndUpdateBvnOnMonnify($kycData){
			...
			if($shouldupdateStatus == "success"){
				...
				$sql = "
					...
					UPDATE subscribers SET sWallet=@newwallet,sKycStatus='verified',sBvn=:bvn,sDob=:dob,sAccountLimit = '10000' WHERE sId=:id;
					INSERT INTO transactions SET 
					sId=:id,transref=:ref,servicename=:sn,servicedesc=:sd,
					amount=:a,status=:s,oldbal=@oldwallet,newbal=@newwallet,date=:d;
...
//create Aspfiy Virtual Account
        ...
        // Create Billstack Palmpay Bank Account
public function generateBillstackAccount($id, $fname, $lname, $phone, $email) {
    // Get Authorization Data
    ...
    // Get API Details
    $d = $this->getApiConfiguration();
    $billstackSecret = $this->getConfigValue($d, "billstackSecret");
    ...
    $result = curl_exec($ch);
    curl_close($ch);
    $value = json_decode($result, true); // Decode JSON response into associative array

    file_put_contents("billstack_log.txt", $result);

    // Check and save account details
    if (isset($value['data']['account'][0]['account_number'])) {
        $accountNumber = $value['data']['account'][0]['account_number'];

        // Save Account Number
        $dbh = $this->connect();
        $c = "UPDATE subscribers SET sPalmpayBank=:psb WHERE sId=$id";
        $queryC = $dbh->prepare($c);
        $queryC->bindParam(':psb', $accountNumber, PDO::PARAM_STR);
        $queryC->execute();
    }
        }
